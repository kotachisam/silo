...
3. **`down`: stop the meter before saving logs.** `cmd_down` runs `try_auto_capture_logs` (SSH `cat /var/log/*.log`) *before* `provider.destroy()`, with no timeout. A slow/hung SSH blocks the destroy that stops billing. **DONE 2026-06-03** — fixed at the SSH layer (`ssh.rs::apply_hardening`): `ConnectTimeout=15` + `ServerAliveInterval=5`/`ServerAliveCountMax=3` bound every ssh call to ~15s, so log-capture fails best-effort and destroy always proceeds. Same change also closes the host-key-prompt backlog item (`BatchMode=yes` + `StrictHostKeyChecking=accept-new`).
4. **`down`: clear local state on `no_such_instance`/404.** Repro: kill an instance externally (dashboard), then `silo down` → `provider.destroy()` returns 404 `no_such_instance` → the `?` aborts `cmd_down` before `state.save()` → the instance lingers in `active.json` forever and `silo status` shows a ghost (`state: unknown`). Fix: add `is_missing_instance_error` (mirror `is_stale_offer_error`); when destroy returns it, log and proceed to `state.save()` — the instance being gone *is* the goal. **DONE + live-verified 2026-06-03.**
5. **`--image`/`--disk`/`--boot` don't bypass a missing profile.** `resolve_up` resolves `--profile` or `default_profile`, and bails if that profile name isn't defined — *before* applying inline flags. So with `default_profile = "vllm"` (a profile that doesn't exist), `silo up <id> --image ubuntu:22.04 --disk 30` fails with "profile 'vllm' not found", despite the error message promising those exact flags as the escape hatch. Fix: when inline `--image`/`--disk`/`--boot` are supplied, fall back to `UpProfile::default()` instead of bailing — or at minimum stop the error message from claiming a bypass that doesn't work.
6. **No way to tell which `silo` binary is running (DX).** The installed `~/.cargo/bin/silo` silently lags the source between `cargo install` runs; stale-binary confusion cost two test cycles on 2026-06-03 (testing old behaviour, assuming the code was wrong). Fix: stamp `silo --version` with the git short-SHA/build time, and/or document running via `cargo run --` during active development so commands always reflect current source.
